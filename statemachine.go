...
const (
	...
	// ↑：连接关闭和任何错误都会导致channel关闭，所以它没有任何事件类型
)
...
type upcallEvent struct {
	...
	// 向 upcallEventHandshakeComplete和upcallEventData发送
	cm *contextManager

	// abstractSyntaxUID 是由P_DATA_TF包导出
	// transferSyntaxUID 是由abstractSyntaxUID商定的值
	// 三次握手期间，双方都不为空
...
var actionDt1 = &stateAction{"DT-1", "Send P-DATA-TF PDU",
	func(sm *stateMachine, event stateEvent) stateType {
		...
		if e.Error() != nil {
			logrus.Panic(fmt.Sprintf("Failed to encode DIMSE cmd %v: %v", command, e.Error()))
		}
		...
		if command.HasData() {
			...
			logrus.Panic(fmt.Sprintf("dicom.stateMachine(%s): Found DIMSE data of %db, command: %v", sm.label, len(event.dimsePayload.data), command))
		}
		return sta06
	}}
...
func networkReaderThread(ch chan stateEvent, conn net.Conn, maxPDUSize int, smName string) {
	...
	for {
		...
		if err != nil {
			logrus.Warnf("dicom.StateMachine %s: 读取PDU失败（EOF表明读取完成）: %v", smName, err)
			if err == io.EOF {
...
func runStateMachineForServiceUser(
	...
	sm := &stateMachine{
		...
		upcallCh:       upcallCh,
		faults:         getUserFaultInjector(), // TODO 把这个干掉 这个是test用
	}
	...
	logrus.Infof("dicom_server.StateMachine(%s): statemachine 完成", sm.label)
}
