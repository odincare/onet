...
func (v *CEchoRsp) String() string {
	return fmt.Sprintf("CEchoRsp{MessageIDBeingRespondedTo:%v CommandDataSetType:%v Status:%v}}",
...
type CFindRq struct {
	...
	Priority            uint16
	CommandDataSetType  uint16
	Extra               []*dicom.Element // 未读取(unparsed)的element
}

func (v *CFindRq) Encode(e *dicomio.Encoder) {
	elements := []*dicom.Element{}
	elements = append(elements, newElement(dicomtag.CommandField, uint16(32)))
	elements = append(elements, newElement(dicomtag.AffectedSOPClassUID, v.AffectedSOPClassUID))
	elements = append(elements, newElement(dicomtag.MessageID, v.MessageID))
	elements = append(elements, newElement(dicomtag.Priority, v.Priority))
	elements = append(elements, newElement(dicomtag.CommandDataSetType, v.CommandDataSetType))
	elements = append(elements, v.Extra...)
	encodeElements(e, elements)
}
...
func (v *CFindRq) String() string {
	return fmt.Sprintf("CFindRq{AffectedSOPClassUID:%v MessageID:%v Priority:%v CommandDataSetType: %v}}",
...
func (v *CFindRsp) Encode(e *dicomio.Encoder) {
	elements := []*dicom.Element{}
	elements = append(elements, newElement(dicomtag.CommandField, uint16(32800)))
	elements = append(elements, newElement(dicomtag.AffectedSOPClassUID, v.AffectedSOPClassUID))
	elements = append(elements, newElement(dicomtag.MessageIDBeingRespondedTo, v.MessageIDBeingRespondedTo))
	elements = append(elements, newElement(dicomtag.CommandDataSetType, v.CommandDataSetType))
	elements = append(elements, newStatusElements(v.Status)...)
	elements = append(elements, v.Extra...)
	encodeElements(e, elements)
}
...
func (v *CFindRsp) String() string {
	return fmt.Sprintf("CFindRsp{AffectedSOPClassUID: %v, MessageIDBeingRespondedTo: %v, CommandDataSetType %v, Status: %v}",
...
type CGetRq struct {
	...
	Priority            uint16
	CommandDataSetType  uint16
	Extra               []*dicom.Element // 未读取的elements
}

func (v *CGetRq) Encode(e *dicomio.Encoder) {
	elements := []*dicom.Element{}
	elements = append(elements, newElement(dicomtag.CommandField, uint16(16)))
	elements = append(elements, newElement(dicomtag.AffectedSOPClassUID, v.AffectedSOPClassUID))
	elements = append(elements, newElement(dicomtag.MessageID, v.MessageID))
	elements = append(elements, newElement(dicomtag.Priority, v.Priority))
	elements = append(elements, newElement(dicomtag.CommandDataSetType, v.CommandDataSetType))
...
func (v *CGetRq) String() string {
	return fmt.Sprintf("CGetRq{AffectedSOPClassUID:%v MessageID:%v Priority:%v CommandDataSetType:%v}}",
		v.AffectedSOPClassUID, v.MessageID, v.Priority, v.CommandDataSetType)
}
...
type CGetRsp struct {
	...
	MessageIDBeingRespondedTo      MessageID
	CommandDataSetType             uint16
	NumberOfRemainingSuboperations uint16           // 还剩多少
	NumberOfCompletedSuboperations uint16           // 完成多少
	NumberOfFailedSuboperations    uint16           // 失败多少
	NumberOfWarningSuboperations   uint16           // 警告多少
	Status                         Status           // 状态
	Extra                          []*dicom.Element // 未读取的elements
}
...
func (v *CGetRsp) Encode(e *dicomio.Encoder) {
	elements := []*dicom.Element{}
	elements = append(elements, newElement(dicomtag.CommandField, uint16(32784)))
	elements = append(elements, newElement(dicomtag.AffectedSOPClassUID, v.AffectedSOPClassUID))
	elements = append(elements, newElement(dicomtag.MessageIDBeingRespondedTo, v.MessageIDBeingRespondedTo))
	elements = append(elements, newElement(dicomtag.CommandDataSetType, v.CommandDataSetType))

	if v.NumberOfRemainingSuboperations != 0 {
		elements = append(elements, newElement(dicomtag.NumberOfRemainingSuboperations, v.NumberOfRemainingSuboperations))
	}

	if v.NumberOfCompletedSuboperations != 0 {
		elements = append(elements, newElement(dicomtag.NumberOfCompletedSuboperations, v.NumberOfCompletedSuboperations))
	}

	if v.NumberOfFailedSuboperations != 0 {
		elements = append(elements, newElement(dicomtag.NumberOfFailedSuboperations, v.NumberOfFailedSuboperations))
	}

	if v.NumberOfWarningSuboperations != 0 {
		elements = append(elements, newElement(dicomtag.NumberOfWarningSuboperations, v.NumberOfWarningSuboperations))
	}
...
func (v *CGetRsp) String() string {
	return fmt.Sprintf("CGetRsp{AffectedSOPClassUID:%v MessageIDBeingRespondedTo:%v CommandDataSetType:%v NumberOfRemainingSuboperations:%v NumberOfCompletedSuboperations:%v NumberOfFailedSuboperations:%v NumberOfWarningSuboperations:%v Status:%v}}",
		v.AffectedSOPClassUID, v.MessageIDBeingRespondedTo, v.CommandDataSetType, v.NumberOfRemainingSuboperations, v.NumberOfCompletedSuboperations, v.NumberOfFailedSuboperations, v.NumberOfWarningSuboperations, v.Status)
}
...
func (v *CMoveRq) Encode(e *dicomio.Encoder) {
	elements := []*dicom.Element{}
	elements = append(elements, newElement(dicomtag.CommandField, uint16(33)))
	elements = append(elements, newElement(dicomtag.AffectedSOPClassUID, v.AffectedSOPClassUID))
	elements = append(elements, newElement(dicomtag.MessageID, v.MessageID))
	elements = append(elements, newElement(dicomtag.Priority, v.Priority))
	elements = append(elements, newElement(dicomtag.MoveDestination, v.MoveDestination))
	elements = append(elements, newElement(dicomtag.CommandDataSetType, v.CommandDataSetType))
	elements = append(elements, v.Extra...)
	encodeElements(e, elements)
}
...
func (v *CMoveRq) String() string {
	return fmt.Sprintf("CMoveRq{AffectedSOPClassUID:%v MessageID:%v Priority:%v MoveDestination:%v CommandDataSetType:%v}}",
...
func (v *CMoveRsp) Encode(e *dicomio.Encoder) {
	elements := []*dicom.Element{}
	elements = append(elements, newElement(dicomtag.CommandField, uint16(32801)))
	elements = append(elements, newElement(dicomtag.AffectedSOPClassUID, v.AffectedSOPClassUID))
	elements = append(elements, newElement(dicomtag.MessageIDBeingRespondedTo, v.MessageIDBeingRespondedTo))
	elements = append(elements, newElement(dicomtag.CommandDataSetType, v.CommandDataSetType))
...
func (v *CMoveRsp) String() string {
	return fmt.Sprintf("CMoveRsp{AffectedSOPClassUID:%v MessageIDBeingRespondedTo:%v CommandDataSetType:%v NumberOfRemainingSuboperations:%v NumberOfCompletedSuboperations:%v NumberOfFailedSuboperations:%v NumberOfWarningSuboperations:%v Status:%v}}",
		v.AffectedSOPClassUID, v.MessageIDBeingRespondedTo, v.CommandDataSetType, v.NumberOfRemainingSuboperations, v.NumberOfCompletedSuboperations, v.NumberOfFailedSuboperations, v.NumberOfWarningSuboperations, v.Status)
}
...
func (v *CStoreRq) String() string {
	return fmt.Sprintf("CStoreRq{AffectedSOPClassUID:%v MessageID:%v Priority:%v CommandDataSetType:%v AffectedSOPInstanceUID:%v MoveOriginatorApplicationEntityTitle:%v MoveOriginatorMessageID:%v}}",
...
func (v *CStoreRsp) String() string {
	return fmt.Sprintf("CStoreRsp{AffectedSOPClassUID:%v MessageIDBeingRespondedTo:%v CommandDataSetType:%v AffectedSOPInstanceUID:%v Status:%v}}",
